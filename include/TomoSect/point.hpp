...
public:
    ...
    template <typename... Args, typename std::enable_if_t<Size == sizeof...(Args), int> = 0>
    explicit point(Args... args) : data_(args...)
    {
    }

    // Construct a non homogeneous vector from a homogeneous one
    explicit point(enoki::Array<Value, Size + 1> homogeneous) : data_(enoki::head<Size>(homogeneous)) {}

    explicit point(vector<Size + 1, Value> homogeneous) : data_(enoki::head<Size>(homogeneous.data())) {}

    // Constructor with a Value, only if Value is an enoki::Array
    template <typename Array = Value, typename std::enable_if_t<enoki::is_array_v<Array>, int> = 0>
    ...
    friend vector<Size, Value> operator-(const point<Size, Value>& lhs, const point<Size, Value>& rhs)
    {
        Vector v(lhs.data_);
        v -= rhs.data_;
...
template <size_t Size, typename Value>
point<Size, Value> operator+(const point<Size, Value>& lhs, const vector<Size, Value>& rhs)
{
    auto copy = lhs;
    copy += rhs;
    return copy;
...
using point2 = point<2, Value>;

template <typename Value>
using point3 = point<3, Value>;

template <typename Value>
using point4 = point<4, Value>;