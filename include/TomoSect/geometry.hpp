...
public:
  ...
  ray(const point3<Value> &o, const vec3<Value> &d)
      : origin_(o), direction_(normalize(d)) {}

  ray(const point3<Value> &o, const vec3<Value> &d, ray_direction_normalized)
...
template <typename Matrix, typename Vector3,
          enoki::enable_if_t<Matrix::IsMatrix && Matrix::Size == 4> = 0>
std::tuple<Matrix, Matrix>
affine_transformation(const Vector3 &translation, const Vector3 &scale,
                      radian<enoki::entry_t<Matrix>> rotX,
                      radian<enoki::entry_t<Matrix>> rotY,
                      radian<enoki::entry_t<Matrix>> rotZ) {
  ...
  Matrix matrixTranslate =
      enoki::translate<Matrix>(Vector3{0.5, 0.5, 0} - translation);

  // From global to local, so scale, then rotate, then move
  auto inv_ =
      matrixTranslate * matrixRotZ * matrixRotY * matrixRotX * matrixScale;
...
template <typename Matrix, typename Vector3,
          enoki::enable_if_t<Matrix::IsMatrix && Matrix::Size == 4> = 0>
std::tuple<Matrix, Matrix>
cylindrical_transformation(const Vector3 &center, const Vector3 &principalPoint,
  ...
  auto matrixScale =
      enoki::scale<Matrix>(Vector3{radius, radius, height_scale});

  // We assume detector is in z direction, x is towards detector and y "right"
  auto rotate = Matrix::from_rows(Vector4{0, 1, 0, 0}, Vector4{0, 0, 1, 0},
                                  Vector4{-1, 0, 0, 0}, Vector4{0, 0, 0, 1});

  auto look_at = enoki::look_at<Matrix>(Vector3{0}, principalPoint - center,
                                        Vector3{0, 1, 0});
...
template <typename Value> class aabb {
public:
  ...
  aabb(const point3<Value> &t, const vec3<Value> &s)
      : m_(enoki::translate<Matrix>(-t.data()) *
           enoki::scale<Matrix>(1 / s.data())),
        inv_(enoki::translate<Matrix>(t.data()) *
...
template <typename Value> class rectangle {
public:
...
public:
  rectangle()
      : pixels_(1), m_(enoki::identity<Matrix4x4>()),
        inv_(enoki::identity<Matrix4x4>()), normal_(calcNormal()) {}

  rectangle(const point2<Value> &pixels, Radian rotX, Radian rotY, Radian rotZ)
      : pixels_(pixels), m_(), inv_(), normal_(0) {
    auto [m, inv] = affine_transformation<Matrix4x4>(Vector3{0}, Vector3{1},
                                                     rotX, rotY, rotZ);
  ...
  rectangle(const point2<Value> &pixels, const vec3<Value> &translation,
            Radian rotX, Radian rotY, Radian rotZ)
      : pixels_(pixels), m_(), inv_(), normal_(0) {
    auto [m, inv] = affine_transformation<Matrix4x4>(
        translation.data(), Vector3{1}, rotX, rotY, rotZ);
  ...
  rectangle(const point2<Value> &pixels, const vec3<Value> &translation,
            const vec3<Value> &scale, Radian rotX, Radian rotY, Radian rotZ)
      : pixels_(pixels), m_(), inv_(), normal_(0) {
    auto [m, inv] = affine_transformation<Matrix4x4>(
        translation.data(), scale.data(), rotX, rotY, rotZ);
  ...
  rectangle(const point2<Value> &pixels, const vec3<Value> &translation,
            Scalar scale, Radian rotX, Radian rotY, Radian rotZ)
      : pixels_(pixels), m_(), inv_(), normal_(0) {
    auto [m, inv] = affine_transformation<Matrix4x4>(
        translation.data(), Vector3{scale}, rotX, rotY, rotZ);
  ...
  rectangle(const point2<Value> &pixels, const point2<Value> &t,
            const vec2<Value> &s)
      : pixels_(pixels), m_(), inv_(), normal_(0) {
  ...
  point3<Value> coordFromLocal(const point2<Value> &p) const {
    enoki::Array<Value, 2> pixelCenter =
        (p.data() + point2<Value>{0.5, 0.5}.data()) / pixels_.data();
    return point3<Value>(
        m_ * enoki::concat(pixelCenter, enoki::Array<Value, 2>{0, 1}));
  }
...
template <typename Value> class curved_rectangle {
public:
  using Matrix4x4 = enoki::Matrix<Value, 4>;
  using Matrix3x3 = enoki::Matrix<Value, 3>;
...
public:
   ...
   * @param theta
   */
  curved_rectangle(const point2<Value> &pixels,
                   const point3<Value> &principalPoint,
                   Scalar height_scale = Scalar{1},
                   Angle theta = degree<float>(180))
      : pixels_(pixels), theta_(theta.get() / 2), m_(), inv_() {

    auto [m, inv] = cylindrical_transformation<Matrix4x4>(
        Vector3{0}, principalPoint.data(), height_scale);

    m_ = m;
    inv_ = inv;
  }

  curved_rectangle(const point2<Value> &pixels, const point3<Value> &center,
                   const point3<Value> &principalPoint,
                   Scalar height_scale = Scalar{1},
                   Angle theta = degree<float>(180))
      : pixels_(pixels), theta_(theta.get() / 2), m_(), inv_() {
    ...
    auto [m, inv] = cylindrical_transformation<Matrix4x4>(
        center.data(), principalPoint.data(), 1);
