...
template <typename Value>
point3<Value> toCartesian(const point3<Value>& p)
{
    // Assume Cylindrical coordinates and convert to cartesian
    // Accessor are: x => r, y => Theta, z => z
    auto raw = p.data();
    return point3<Value>{ p.x() * enoki::cos(p.y()), p.x() * enoki::sin(p.y()), p.z() };
}

template <typename Value>
point3<Value> toCylindrical(const point3<Value>& p)
{
    // Assume cartesian coordinates and convert to cylindrical
    auto raw = p.data();
    return point3<Value>{ enoki::sqrt(enoki::pow(p.x(), 2) + enoki::pow(p.y(), 2)), enoki::atan(p.y() / p.x()), p.z() };
}

template <typename Matrix, typename Vector3, enoki::enable_if_t<Matrix::IsMatrix && Matrix::Size == 4> = 0>
std::tuple<Matrix, Matrix> affine_transformation(const Vector3& translation, const Vector3& scale, radian<enoki::entry_t<Matrix>> rotX,
...
template <typename Matrix, typename Vector3, enoki::enable_if_t<Matrix::IsMatrix && Matrix::Size == 4> = 0>
std::tuple<Matrix, Matrix> cylindrical_transformation(const Vector3& center, const Vector3& principalPoint,
                                                      enoki::entry_t<Matrix> height_scale)
{
    ...
    auto rotate = Matrix::from_rows(Vector4{ 0, 1, 0, 0 }, Vector4{ 0, 0, 1, 0 }, Vector4{ -1, 0, 0, 0 }, Vector4{ 0, 0, 0, 1 });
...
public:
    ...
    aabb(const point3<Value>& min, const point3<Value>& max) : aabb(min, max - min) {}

    point3<Value> min() const { return point3<Value>(inv_ * enoki::Array<Value, 4>{ 0, 0, 0, 1 }); }

    point3<Value> max() const { return point3<Value>(inv_ * enoki::Array<Value, 4>{ 1, 1, 1, 1 }); }
...
public:
    rectangle() : pixels_(1), m_(enoki::identity<Matrix4x4>()), inv_(enoki::identity<Matrix4x4>()), normal_(calcNormal()) {}

    rectangle(const point2<Value>& pixels, Radian rotX, Radian rotY, Radian rotZ) : pixels_(pixels), m_(), inv_(), normal_(0)
    {
    ...
    rectangle(const point2<Value>& pixels, const vec3<Value>& translation, Radian rotX, Radian rotY, Radian rotZ)
        : pixels_(pixels), m_(), inv_(), normal_(0)
    {
    ...
    rectangle(const point2<Value>& pixels, const vec3<Value>& translation, const vec3<Value>& scale, Radian rotX, Radian rotY, Radian rotZ)
        : pixels_(pixels), m_(), inv_(), normal_(0)
    {
    ...
    rectangle(const point2<Value>& pixels, const vec3<Value>& translation, Scalar scale, Radian rotX, Radian rotY, Radian rotZ)
        : pixels_(pixels), m_(), inv_(), normal_(0)
    {
    ...
    rectangle(const point2<Value>& pixels, const point2<Value>& t, const vec2<Value>& s) : pixels_(pixels), m_(), inv_(), normal_(0)
    {
        /*enoki::Array<Value, 3> tmp_scale = enoki::concat(1 / s.data(),
        enoki::Array<Value, 1>{1});

        auto tmp_scale_2d = enoki::scale<Matrix3x3>(1 / s.data());
        auto tmp_translate_2d = enoki::translate<Matrix3x3>(enoki::Array<Value,
        2>{0.5, 0.5} - t.data());
        ...
        enoki::Array<Value, 3> concat = enoki::concat(t.data(), enoki::Array<Value,
        1>{0}); enoki::Array<Value, 3> tmp = base - concat; auto translate_2d =
        enoki::translate<Matrix4x4>(tmp);*/
        // auto translation_3d = enoki::identity<Matrix>();
        // auto rotate_3d = enoki::identity<Matrix>();

        /*
         * enoki::scale<Matrix>(1 / s.data()) *
                 (enoki::translate<Matrix>(enoki::Array<Value, 3>{0.5, 0.5, 0} -
         t.data())),
    ...
    point3<Value> min() const { return point3<Value>(m_ * enoki::Array<Value, 4>{ 0, 0, 0, 1 }); }

    point3<Value> max() const { return point3<Value>(m_ * enoki::Array<Value, 4>{ 1, 1, 0, 1 }); }

    point3<Value> center() const { return point3<Value>(m_ * enoki::Array<Value, 4>{ 0.5, 0.5, 0, 1 }); }

    vec3<Value> normal() const { return normal_; }
...
public:
    curved_rectangle() : pixels_(1), m_(enoki::identity<Matrix4x4>()), inv_(enoki::identity<Matrix4x4>()), theta_(Value{ 45 }), aabb_() {}
    ...
    curved_rectangle(const point2<Value>& pixels, const point3<Value>& principalPoint, Scalar height_scale = Scalar{ 1 },
                     Angle theta = degree<Value>(180))
        : pixels_(pixels), theta_(theta.get() / 2), m_(), inv_()
    {
    ...
    curved_rectangle(const point2<Value>& pixels, const point3<Value>& center, const point3<Value>& principalPoint,
                     Scalar height_scale = Scalar{ 1 }, Angle theta = degree<Value>(180))
        : pixels_(pixels), theta_(theta.get() / 2), m_(), inv_()
    {
    ...
    point3<Value> coordFromLocal(const point2<Value>& p) const
    {
        ...
        auto local = Vector4{ enoki::cos(phi_rad), enoki::sin(phi_rad), z, 1 };
    ...
    point3<Value> principal_point() const { return point3<Value>(m_ * Vector4{ 1, 0, 0, 1 }); }

    point3<Value> center() const { return point3<Value>(m_ * Vector4{ 0, 0, 0, 1 }); }

    auto angle() const { return theta_.to_radian(); }

    template <typename Vec>
    Vec toLocal(const Vec& p) const
    {
        auto vec4 = enoki::concat(p.data(), enoki::Array<Value, 1>{ 1 });
        return Vec(enoki::head<3>(inv_ * vec4));
    }
    ...
    {
        auto vec4 = enoki::concat(p.data(), enoki::Array<Value, 1>{ 1 });
        return Vec(enoki::head<3>(m_ * vec4));
    }

    const aabb<Value>& getAABB() const { return aabb_; }

private:
    void buildAABB()
    {
        // Construct the AABB first TODO move to curved_rectangle
        auto min = coordFromLocal(point2<Value>{ 0, 1 });

        auto p = principal_point();
        auto upper_right = coordFromLocal(point2<Value>{ 1, 0 });

        auto max = point3<Value>{ upper_right.x(), upper_right.y(), p.z() };

        aabb_ = aabb<Value>(min, max);
    }

    point2<Value> pixels_; // Number of pixels
    Matrix4x4 m_;          // Local to Global matrix
    Matrix4x4 inv_;        // Global to Local matrix
    Angle theta_;          // angle of between line from center to principal point and one
                           // outer edge with z = 0
    aabb<Value> aabb_;
};