...
template <size_t Size, typename Value> class vector {
public:
  ...
  std::enable_if_t<enoki::is_array_v<V>, Self &> operator+=(const Value &rhs) {
    data_ += rhs;
    return *this;
  }
...
template <size_t Size, typename Value>
vector<Size, Value> cross(const vector<Size, Value> &lhs,
...
template <size_t Size, typename Value>
vector<Size, Value> operator+(const vector<Size, Value> &lhs,
                              const vector<Size, Value> &rhs) {
  auto copy = lhs;
  copy += rhs;
  return copy;
...
template <size_t Size, typename Value,
          typename std::enable_if_t<enoki::is_array_v<Value>, int> = 0>
vector<Size, Value> operator+(const vector<Size, Value> &lhs,
                              const Value &rhs) {
  auto copy = lhs;
  copy += rhs;
...
template <size_t Size, typename Value>
vector<Size, Value> operator+(const vector<Size, Value> &lhs,
                              const typename vector<Size, Value>::Type &rhs) {
  auto copy = lhs;
  copy += rhs;
  return copy;
}
...
template <size_t Size, typename Value>
vector<Size, Value> operator+(const typename vector<Size, Value>::Type &lhs,
                              const vector<Size, Value> &rhs) {
  return rhs + lhs;
}
...
template <size_t Size, typename Value>
vector<Size, Value> operator*(const Value &lhs,
                              const vector<Size, Value> &rhs) {
  return rhs + lhs;
}
...
template <typename Value> using vec2 = vector<2, Value>;

template <typename Value> using vec3 = vector<3, Value>;

template <typename Value> using vec4 = vector<4, Value>;

template <typename T> using pack2 = enoki::Packet<T, 2>;

template <typename T> using pack4 = enoki::Packet<T, 4>;

template <typename T> using pack8 = enoki::Packet<T, 8>;

template <typename T> using pack16 = enoki::Packet<T, 16>;

template <typename T> using pack32 = enoki::Packet<T, 32>;

template <typename T> using pack64 = enoki::Packet<T, 64>;