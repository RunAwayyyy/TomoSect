...
public:
  ...
  static void resetCount() { CopyCountingInt::copiesCounted = 0; }

  static size_t getCount() { return CopyCountingInt::copiesCounted; }
...
public:
  class CopyCountUDM : public celero::UserDefinedMeasurementTemplate<size_t> {
    virtual std::string getName() const override { return "Copies"; }
...
BASELINE_F(SortRandInts, BubbleSort, SortFixture, SamplesCount,
           IterationsCount) {
...
BENCHMARK_F(SortRandInts, SelectionSort, SortFixture, SamplesCount,
            IterationsCount) {
  for (int x = 0; x < this->arraySize; x++) {
    auto minIdx = x;
...
BENCHMARK_F(SortRandInts, InsertionSort, SortFixture, SamplesCount,
            IterationsCount) {
  for (auto i = std::begin(this->array); i != std::end(this->array); ++i) {
    std::rotate(std::upper_bound(std::begin(this->array), i, *i), i,
                std::next(i));
  }
}
...
BENCHMARK_F(SortRandInts, QuickSort, SortFixture, SamplesCount,
            IterationsCount) {
