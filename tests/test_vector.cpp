...
static_assert(std::is_same_v<vec2<pack2<float>>::Type, float>,
              "Expect \'Type\' to be float");
  ...
  SUBCASE("Testing Math functions") {
    ...
    SUBCASE("Dot product") {
      SUBCASE("2D Vector") {
        ...
        static_assert(std::is_same_v<decltype(d), pack2<float>>,
                      "Return type of dot product is a pack");
